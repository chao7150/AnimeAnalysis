...
def calc_match_bpm(data,bpm):
    ...
    sin_match   = (1/N) * sum( data[:,0] * np.sin(phase_array))
    cos_match   = (1/N) * sum( data[:,0] * np.cos(phase_array))
...
if __name__ == '__main__':
    ...
    frame_list = np.vsplit(dt[:sample_max], frame_max)
